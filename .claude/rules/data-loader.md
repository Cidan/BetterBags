...
- **Targeted Merging:** In `ProcessRefresh`, when a targeted sweep completes, newly harvested `itemData` for the targeted bags is merged with `previousItems`: slot keys belonging to targeted bags are cleared from `previousItems`, and newly harvested slots are inserted. The resulting complete, unified `itemData` dictionary is then passed down to downstream phases (virtual stacking, search indexing, category enrichment, sorting, tab partitioning), ensuring downstream phases always operate on a 100% complete model of all bags.
