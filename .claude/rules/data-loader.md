...
- **Mechanism:** Immediately after Phase 4's search indexing `search:IndexItems(itemData)` completes (`items:Phase8_EnrichCategories(ctx, kind, itemData, emptySlotByBagAndSlot)`):
  ...
  - **Live Search Results:** The active search box text is evaluated here as well (`data.isSearchResult`), never in Phase 6. Phase 6 runs before `search:IndexItems`, so evaluating there queries the previous sweep's index and newly looted or moved items never match until the following refresh.
