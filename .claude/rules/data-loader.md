...
- **Pure Data-Driven physical layout (Show Bags / `SECTION_ALL_BAGS`):** The data phase unconditionally pre-computes both the `categoryView` and `bagView` layouts under `slotInfo.layouts`. The `bagView` layout represents items under their physical bag categories using a lightweight `setmetatable` proxy, avoiding any mutation of the original items and completely decoupling UI state from raw data.
- **Synchronous, State-Driven Toggle:** Swapping between "Show Bags" and standard Category view is fully decoupled from database rebuilds or full scans. Toggle buttons set `tab_switch = true` on the context and immediately call `bag:Draw` using the statically pre-computed `layouts.bagView` or `layouts.categoryView` layout. This achieves sub-millisecond local transition latency with zero database queries.
