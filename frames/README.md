...
- Left-click on an **unpurchased** tab opens the Blizzard bank tab purchase dialog via secure click forwarding: each slot button is created with `InsecureActionButtonTemplate` and each has its own dedicated hidden child `BankPanelPurchaseButtonScriptTemplate` button with `overrideBankType` set to the appropriate `Enum.BankType` value. When unpurchased, the slot button's `type=click` and `clickbutton` attributes route hardware clicks through the child purchase button, causing `BankPanelPurchaseTabButtonMixin:OnClick` to execute in clean (Blizzard) code context and keeping the `bankType` argument untainted. The slot button uses `RegisterForClicks("LeftButtonDown", "RightButtonDown")` when unpurchased — matching the old bottom-tab mechanism — because `SecureActionButton_OnClick` fires the forwarding action on `down=true` with the `ActionButtonUseKeyDown` CVar (which defaults to true in modern retail WoW).
