...
--- Classic Era doesn't have the currency system (GetCurrencyListInfo API), so the
--- currency grid is intentionally omitted. The unified frames/bag.lua delegates
--- search/slots/theme-config creation to the behavior's OnCreate (the legacy
--- frames/era/bag.lua that created them inline was removed in the UI unification,
--- #1044), so this override must create them itself — otherwise bag.slots is never
--- set and the context menu's "Show Bags" entry (gated on bag.slots) disappears.
-------
...
function backpack.proto:OnCreate(ctx)
	...
	-- Theme config (currency grid omitted: no currency API in Era)
