...
When `GetShowBankTabs()` is true, `OnShow()` pre-configures `bag.blizzardBankTab` and the panel's selected state *before* any render occurs (before the fade/no-fade split). This mirrors the backpack's 'show bags' pattern: the correct display state is established upfront so that the first `Bank:Draw()` call from `init.lua` already renders with the tab filter applied. Without this, the initial render would show the full grouped bank for the duration of the panel's fade-in animation — a visible flash of unstyled content. The panel's `SelectFirstTab()` call (triggered by the fade-in `OnFinished` hook) checks `selectedBagIndex` and skips the redundant re-render when the tab is already pre-selected.
