...
function currencyPaneProto:UpdateDetailPanel()
  ...
  local info = C_CurrencyInfo.GetCurrencyListInfo(self.selectedIndex)
...
function currencyPaneProto:CreateCurrencyDetailPanel()
  ...
  toggleButton:SetScript("OnClick", function()
    if self.currencyDetail.currentIndex then
      local info = C_CurrencyInfo.GetCurrencyListInfo(self.currencyDetail.currentIndex)
      if info then
        C_CurrencyInfo.SetCurrencyBackpack(self.currencyDetail.currentIndex, not info.isShowInBackpack)
...
function currencyPaneProto:RefreshList()
  ...
  for i = 1, listSize do
    ...
    if info then
      -- Expand headers so we can see all currencies
      if info.isHeader and not info.isHeaderExpanded then
        C_CurrencyInfo.ExpandCurrencyList(i, true)
        -- Re-fetch after expansion as indices may have changed
        self:RefreshList()
        return
      end
